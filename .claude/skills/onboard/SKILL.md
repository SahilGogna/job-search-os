---
name: onboard
description: Use the first time this repo is opened (no context/profile.md exists yet), or whenever the candidate wants to update their profile from a new/revised resume ("update my resume", "onboard me", "set up my profile", "I have a new resume"). One-time setup that reads a full multi-page resume, proposes target roles and confirms them with the candidate before locking them in, and produces context/profile.md + configs/search.json. Re-running it later diffs the new resume against the existing profile and asks what to accept, rather than silently overwriting.
---
...
- `location.region_aliases`: spelling variants of the province LinkedIn may use in posting-location strings.
...
One-line confirmation of what was written, then point the candidate at the job-search skill: "Say 'find jobs for me' whenever you want to run a search."
