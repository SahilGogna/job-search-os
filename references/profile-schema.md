...
- **`core_skills`** — from the profile's skills list, weighted by how central each is to the candidate's story. Keys are snake_case (`power_bi`); `variants` are the natural-language forms matched against posting text.
